...
const CadminUserProfile: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (editingProfile) {
      updateProfile(editingProfile.user_id || editingProfile.id, formData);
    } else {
          ...
          {profiles.map((profile) => (
            ...
            >
              ...
              <div className="space-y-2 mb-4">
                ...
                <div className="flex justify-between">
                  <span className="text-sm" style={{ color: '#6B7280' }}>User ID:</span>
                  <span className="text-xs" style={{ color: '#060C18' }}>
                    {profile.user_id || 'N/A'}
                  </span>
                </div>
              </div>
              ...
              <div className="flex space-x-2">
                ...
                {profile.status !== 'active' && (
                  <button
                    onClick={() => activateUser(profile.user_id || profile.id)}
                    className="flex-1 flex items-center justify-center space-x-2 py-2 px-3 rounded-lg text-white font-medium hover:opacity-90 transition-opacity"
                ...
                <button
                  onClick={() => deleteProfile(profile.user_id || profile.id)}
                  className="flex-1 flex items-center justify-center space-x-2 py-2 px-3 rounded-lg text-white font-medium hover:opacity-90 transition-opacity"
        ...
        {profiles.length === 0 && !loading && (
          <div className="text-center py-12">
            ...
            <p className="text-lg" style={{ color: '#6B7280' }}>
              No profiles found for this property. Create your first profile!
            </p>
          </div>
        )}
      </div>
...
export default CadminUserProfile;