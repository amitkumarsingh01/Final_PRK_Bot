...
const AssetMovementLogPage: React.FC = () => {
  ...
  const handleAdd = async () => {
    ...
    if (!reportId) return;
    setEditModal({ open: true, isNew: true, item: { ...emptyMovementLog }, reportId });
  };
  const handleDelete = async (itemId: string, reportId: string) => {
    ...
    try {
      ...
      await axios.put(`${API_URL}${reportId}`, { movement_logs: newArr }, { headers: { Authorization: `Bearer ${user?.token}` } });
      fetchData();
    } catch (e) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await axios.put(`${API_URL}${editModal.reportId}`, { movement_logs: newArr }, { headers: { Authorization: `Bearer ${user?.token}` } });
