...
const SystemDesignPlanningPage: React.FC = () => {
  console.log('🚀 SystemDesignPlanning: Component initialized');
  const { user } = useAuth();
  console.log('👤 SystemDesignPlanning: User loaded', { userId: user?.userId });
  ...
  const handleAdd = async (reportId?: string) => {
    ...
    if (!id) return;
    setEditModal({ open: true, isNew: true, item: { ...emptySiteAssessment }, reportId: id });
  };
              ...
              <tr>
                <td colSpan={11} className="text-center py-6">
                  <div className="flex items-center justify-center gap-3">
                    ...
                    {isAdmin && (
                      <button onClick={() => handleAdd()} className="ml-2 px-3 py-1 rounded bg-gradient-to-r from-[#E06002] to-[#FB7E03] text-white font-semibold shadow">Add System Design Entry</button>
                    )}
                  </div>
                </td>
              </tr>
