...
import { Pencil, Trash2, Plus, Save, X, Eye, Users, Calendar, FileText, CheckCircle, AlertTriangle, TrendingUp } from 'lucide-react';
  ...
  const fetchData = async () => {
    try {
      ...
      const filteredData = user?.propertyId 
        ? response.data.filter((item: HeightWorkPermit) => item.property_id === user.propertyId)
        : response.data;
      setData(filteredData);
    } catch (e) {
      setError('Failed to fetch height work permits');
    } finally {
      setLoading(false);
    }
  };
