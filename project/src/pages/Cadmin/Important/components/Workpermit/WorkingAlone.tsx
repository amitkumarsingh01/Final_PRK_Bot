...
import { Pencil, Trash2, Plus, Save, X, Eye, Users, Calendar, FileText, CheckCircle, AlertTriangle, UserCheck } from 'lucide-react';
  ...
  const fetchData = async () => {
    try {
      ...
      const filteredData = user?.propertyId 
        ? response.data.filter((item: WorkingAlonePermit) => item.property_id === user.propertyId)
        : response.data;
      setData(filteredData);
    } catch (e) {
      setError('Failed to fetch working alone permits');
    } finally {
      setLoading(false);
    }
  };
