...
import { Pencil, Trash2, Plus, Save, X, Eye, Users, Calendar, FileText, CheckCircle, AlertTriangle, Home } from 'lucide-react';
  ...
  const fetchData = async () => {
    try {
      ...
      const filteredData = user?.propertyId 
        ? response.data.filter((item: TemporaryStructureInstallationPermit) => item.property_id === user.propertyId)
        : response.data;
