...
import { Pencil, Trash2, Plus, Save, X, Eye, Users, Calendar, FileText, CheckCircle, AlertTriangle, ArrowUp } from 'lucide-react';
...
const LiftingPage: React.FC = () => {
  ...
  const handleAdd = () => {
    setEditModal({ open: true, record: { ...emptyLiftingPermit, property_id: user?.propertyId }, isNew: true });
  };
  ...
  const fetchData = async () => {
    try {
      ...
      const filteredData = user?.propertyId 
        ? response.data.filter((item: LiftingWorkPermit) => item.property_id === user.propertyId)
        : response.data;
