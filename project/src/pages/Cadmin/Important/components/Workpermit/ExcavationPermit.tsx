...
import { Pencil, Trash2, Plus, Save, X, Eye, Users, Calendar, FileText, CheckCircle, AlertTriangle, Shovel } from 'lucide-react';
import { useAuth } from '../../../../../context/AuthContext';  


interface ExcavationWorkPermit {
  id?: number;
  property_id: string;
  ...
  site_location_of_excavation: string;
  purpose_of_excavation: string;
  ...
  depth_of_excavation_meters: number;
  length_of_excavation_meters: number;
  width_of_excavation_meters: number;
  soil_type: string;
  underground_utilities_identified: string;
  utilities_marked_and_protected: string;
  shoring_or_sloping_required: string;
  shoring_equipment_installed: string;
...
const API_URL = 'https://server.prktechindia.in/excavation-work-permit/';

const emptyExcavationPermit: ExcavationWorkPermit = {
  property_id: '',
  permit_number: '',
  date_of_issue: '',
  permit_valid_from: '',
  permit_valid_to: '',
  site_location_of_excavation: '',
  purpose_of_excavation: '',
  ...
  depth_of_excavation_meters: 0,
  length_of_excavation_meters: 0,
  width_of_excavation_meters: 0,
  soil_type: '',
  underground_utilities_identified: '',
  utilities_marked_and_protected: '',
  shoring_or_sloping_required: '',
  shoring_equipment_installed: '',
...
const ExcavationPermitPage: React.FC = () => {
  const { user } = useAuth();
  const [data, setData] = useState<ExcavationWorkPermit[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [isAdmin, setIsAdmin] = useState(false);
  const [viewModal, setViewModal] = useState<{ open: boolean; record: ExcavationWorkPermit | null }>({ open: false, record: null });
  const [editModal, setEditModal] = useState<{ open: boolean; record: ExcavationWorkPermit | null; isNew: boolean }>({ open: false, record: null, isNew: false });

  const handleEdit = (record: ExcavationWorkPermit) => {
    setEditModal({ open: true, record: { ...record }, isNew: false });
  };

  const handleAdd = () => {
    setEditModal({ open: true, record: { ...emptyExcavationPermit, property_id: user?.propertyId || '' }, isNew: true });
  };

  const handleDelete = async (recordId: number) => {
    if (!isAdmin) {
      alert('Only admins can delete excavation permits');
      return;
    }
    
    if (window.confirm('Are you sure you want to delete this excavation permit?')) {
      try {
        await axios.delete(`${API_URL}${recordId}`);
        fetchData();
      } catch (e) {
        setError('Failed to delete excavation permit');
      }
    }
  };

  const handleView = (record: ExcavationWorkPermit) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setError('Failed to save excavation permit');
    }
  };
  ...
  const fetchData = async () => {
    try {
      ...
      const filteredData = user?.propertyId 
        ? response.data.filter((item: ExcavationWorkPermit) => item.property_id === user.propertyId)
        : response.data;
      setData(filteredData);
    } catch (e) {
      setError('Failed to fetch excavation permits');
    } finally {
      setLoading(false);
    }
  };
  ...
  const isPermitActive = (permit: ExcavationWorkPermit) => {
    const now = new Date();
  ...
  const getPermitStatus = (permit: ExcavationWorkPermit) => {
        ...
        <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h1 className="text-2xl font-bold text-gray-900">Excavation Work Permits</h1>
                <p className="text-gray-600">Manage excavation work permits and ground safety compliance</p>
              </div>
            </div>
            {isAdmin && (
              <button
              ...
              >
                ...
                <span>Add Excavation Permit</span>
              </button>
            )}
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-sm overflow-hidden">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {data.map((permit) => (
                  <tr key={permit.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{permit.site_location_of_excavation}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{permit.purpose_of_excavation}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                      {permit.depth_of_excavation_meters}m × {permit.length_of_excavation_meters}m × {permit.width_of_excavation_meters}m
                    </td>
          ...
          {data.length === 0 && (
            <div className="text-center py-12">
              <Shovel className="mx-auto h-12 w-12 text-gray-400" />
              <h3 className="mt-2 text-sm font-medium text-gray-900">No excavation permits</h3>
              <p className="mt-1 text-sm text-gray-500">Get started by creating a new excavation permit.</p>
            </div>
          )}
        </div>
      </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-6xl w-full mx-4 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-xl font-bold text-gray-900">Excavation Work Permit Details</h2>
              <button
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div><b>Site Location:</b> {viewModal.record.site_location_of_excavation}</div>
                  <div><b>Purpose:</b> {viewModal.record.purpose_of_excavation}</div>
                </div>
              </div>

              {/* Excavation Details */}
              <div>
                <h3 className="text-lg font-semibold text-gray-900 mb-3">Excavation Details</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div><b>Depth:</b> {viewModal.record.depth_of_excavation_meters} meters</div>
                  <div><b>Length:</b> {viewModal.record.length_of_excavation_meters} meters</div>
                  <div><b>Width:</b> {viewModal.record.width_of_excavation_meters} meters</div>
                  <div><b>Soil Type:</b> {viewModal.record.soil_type}</div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div><b>Underground Utilities Identified:</b> {viewModal.record.underground_utilities_identified}</div>
                  <div><b>Utilities Marked & Protected:</b> {viewModal.record.utilities_marked_and_protected}</div>
                  <div><b>Shoring/Sloping Required:</b> {viewModal.record.shoring_or_sloping_required}</div>
                  <div><b>Shoring Equipment Installed:</b> {viewModal.record.shoring_equipment_installed}</div>
                  <div><b>Barricading & Signages:</b> {viewModal.record.barricading_and_signages_installed}</div>
                  <div><b>Safety Helmet & PPE:</b> {viewModal.record.safety_helmet_and_ppe_worn}</div>
                </div>
              </div>
          ...
          <div className="bg-white rounded-lg p-6 max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-xl font-bold text-gray-900">
                {editModal.isNew ? 'Add New Excavation Permit' : 'Edit Excavation Permit'}
              </h2>
              <button
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    type="text"
                    value={editModal.record.site_location_of_excavation}
                    onChange={(e) => setEditModal({...editModal, record: {...editModal.record!, site_location_of_excavation: e.target.value}})}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500"
                  />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Purpose of Excavation</label>
                  <input
                    type="text"
                    value={editModal.record.purpose_of_excavation}
                    onChange={(e) => setEditModal({...editModal, record: {...editModal.record!, purpose_of_excavation: e.target.value}})}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500"
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  ...
                  <input
                    type="number"
                    step="0.1"
                    value={editModal.record.depth_of_excavation_meters}
                    onChange={(e) => setEditModal({...editModal, record: {...editModal.record!, depth_of_excavation_meters: parseFloat(e.target.value)}})}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500"
                  />
                </div>
                <div>
                  ...
                  <input
                    type="number"
                    step="0.1"
                    value={editModal.record.length_of_excavation_meters}
                    onChange={(e) => setEditModal({...editModal, record: {...editModal.record!, length_of_excavation_meters: parseFloat(e.target.value)}})}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500"
                  />
                </div>
                <div>
                  ...
                  <input
                    type="number"
                    step="0.1"
                    value={editModal.record.width_of_excavation_meters}
                    onChange={(e) => setEditModal({...editModal, record: {...editModal.record!, width_of_excavation_meters: parseFloat(e.target.value)}})}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500"
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    type="text"
                    value={editModal.record.soil_type}
                    onChange={(e) => setEditModal({...editModal, record: {...editModal.record!, soil_type: e.target.value}})}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500"
                  />
                </div>
                <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Underground Utilities Identified</label>
                  <select
                    value={editModal.record.underground_utilities_identified}
                    onChange={(e) => setEditModal({...editModal, record: {...editModal.record!, underground_utilities_identified: e.target.value}})}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500"
                  >
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Utilities Marked & Protected</label>
                  <select
                    value={editModal.record.utilities_marked_and_protected}
                    onChange={(e) => setEditModal({...editModal, record: {...editModal.record!, utilities_marked_and_protected: e.target.value}})}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500"
                  >
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Shoring/Sloping Required</label>
                  <select
                    value={editModal.record.shoring_or_sloping_required}
                    onChange={(e) => setEditModal({...editModal, record: {...editModal.record!, shoring_or_sloping_required: e.target.value}})}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500"
                  >
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Shoring Equipment Installed</label>
                  <select
                    value={editModal.record.shoring_equipment_installed}
                    onChange={(e) => setEditModal({...editModal, record: {...editModal.record!, shoring_equipment_installed: e.target.value}})}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500"
                  >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Barricading & Signages</label>
                  <select
                    value={editModal.record.barricading_and_signages_installed}
                    onChange={(e) => setEditModal({...editModal, record: {...editModal.record!, barricading_and_signages_installed: e.target.value}})}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500"
                  >
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Safety Helmet & PPE</label>
                  <select
                    value={editModal.record.safety_helmet_and_ppe_worn}
                    onChange={(e) => setEditModal({...editModal, record: {...editModal.record!, safety_helmet_and_ppe_worn: e.target.value}})}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500"
                  >
...
export default ExcavationPermitPage;
