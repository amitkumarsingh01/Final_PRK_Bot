...
const CDailyTaskManagementAllDepartment: React.FC = () => {
  ...
  const handleSetStatus = async (period: string, status: string) => {
    ...
    try {
      ...
      // Send to backend
      await axios.post(STATUS_API_URL, {
                ...
                {data.map((item, idx) => (
                  <tr key={item.id} style={{ background: idx % 2 === 0 ? '#fff' : '#FFF7ED' }}>
                    ...
                    <td className="border px-2 py-1">{editId === item.id ? (
                      <select name="frequency" value={editRow?.frequency ?? ''} onChange={handleEditChange} className="w-32 border rounded px-1">
                    ...
                    <td className="border px-2 py-1 text-center">{editId === item.id ? (
                      <select name="user_required" value={editRow?.user_required ? 'true' : 'false'} onChange={handleEditChange} className="w-20 border rounded px-1">
                ...
                {isAdmin && newRow && (
                  <tr style={{ background: '#FFF7ED' }}>
                    ...
                    <td className="border px-2 py-1">
                      <select name="frequency" value={newRow.frequency} onChange={handleNewRowChange} className="w-32 border rounded px-1">
                        <option value="">Select</option>
                        {frequencyOptions.map(option => (
                          <option key={option} value={option}>{option}</option>
                        ))}
                    ...
                    <td className="border px-2 py-1 text-center">
                      <select name="user_required" value={newRow.user_required ? 'true' : 'false'} onChange={handleNewRowChange} className="w-20 border rounded px-1">
                        {userOptions.map(option => (
                          <option key={option.value.toString()} value={option.value.toString()}>{option.label}</option>
                        ))}
                      </select>
