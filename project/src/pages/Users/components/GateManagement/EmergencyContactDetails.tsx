...
const emptyEmergency: EmergencyContactDetails = {
  contact_id: '',
  name: '',
  contact_number: '',
  relation: '',
...
const CEmergencyContactDetailsPageP: React.FC = () => {
  ...
  const handleDelete = async (itemId: string, reportId: string) => {
    ...
    try {
      ...
      if (!report) return;
      const newArr = report.emergency_contact_details.filter(i => i.id !== itemId);
      await axios.put(`${API_URL}${reportId}`, { emergency_contact_details: newArr });
      fetchData();
    } catch (e) {
      setError('Failed to delete');
    }
  };
  const handleView = (item: EmergencyContactDetails) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!report) return;
      let newArr: EmergencyContactDetails[];
      if (editModal.isNew) {
        newArr = [...report.emergency_contact_details, editModal.item];
      } else {
        newArr = report.emergency_contact_details.map(i =>
  ...
  return (
    <div className="p-6" style={{ background: '#fff' }}>
        ...
        <table className="min-w-full text-sm">
          <thead>
            <tr style={{ background: orange, color: '#fff' }}>
              ...
              <th className="px-3 py-2 border">Contact ID</th>
              <th className="px-3 py-2 border">Name</th>
              <th className="px-3 py-2 border">Contact Number</th>
              <th className="px-3 py-2 border">Relation</th>
              <th className="px-3 py-2 border">Address</th>
              <th className="px-3 py-2 border">Emergency Type</th>
            ...
            {loading ? (
              ...
              <>
                {data.flatMap((report, rIdx) =>
                  report.emergency_contact_details.map((item, idx) => (
                    <tr key={item.id || idx} style={{ background: idx % 2 === 0 ? '#fff' : '#FFF7ED' }}>
                      <td className="border px-2 py-1">{idx + 1}</td>
                      <td className="border px-2 py-1">{item.contact_id}</td>
                      <td className="border px-2 py-1">{item.name}</td>
                      <td className="border px-2 py-1">{item.contact_number}</td>
                      <td className="border px-2 py-1">{item.relation}</td>
                      <td className="border px-2 py-1">{item.address}</td>
                      <td className="border px-2 py-1">{item.emergency_type}</td>
      ...
      {isAdmin && data.length > 0 && (
        ...
        >
          <Plus size={18} className="mr-2" /> Add Emergency Contact
        </button>
      )}
          ...
          <div className="bg-white rounded-lg p-6 max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold text-gray-900">
                {editModal.isNew ? 'Add' : 'Edit'} Emergency Contact
              </h3>
              <button
            ...
            <form onSubmit={e => { e.preventDefault(); handleSave(); }}>
              <div className="grid grid-cols-2 gap-3">
                <input className="border rounded px-3 py-2" placeholder="Contact ID" value={editModal.item.contact_id} onChange={e => setEditModal(m => m && { ...m, item: { ...m.item!, contact_id: e.target.value } })} required />
                ...
                <input className="border rounded px-3 py-2" placeholder="Relation" value={editModal.item.relation} onChange={e => setEditModal(m => m && { ...m, item: { ...m.item!, relation: e.target.value } })} required />
                ...
                <input className="border rounded px-3 py-2" placeholder="Emergency Type" value={editModal.item.emergency_type} onChange={e => setEditModal(m => m && { ...m, item: { ...m.item!, emergency_type: e.target.value } })} required />
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              <div><b>Contact ID:</b> {viewModal.item.contact_id}</div>
              <div><b>Name:</b> {viewModal.item.name}</div>
              <div><b>Contact Number:</b> {viewModal.item.contact_number}</div>
              <div><b>Relation:</b> {viewModal.item.relation}</div>
              <div><b>Address:</b> {viewModal.item.address}</div>
              <div><b>Emergency Type:</b> {viewModal.item.emergency_type}</div>
...
export default CEmergencyContactDetailsPageP;
