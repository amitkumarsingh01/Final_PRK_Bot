...
const CWorkPermitIssuancePage: React.FC = () => {
  console.log('?? WorkPermitIssuance: Component initialized');
  const { user } = useAuth();
  console.log('?? WorkPermitIssuance: User loaded', { userId: user?.userId });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editModal.isNew) {
        ...
        newArr = [...report.work_permit_issuance, newEntry];
      } else {
      ...
      console.log('Sending data:', { work_permit_issuance: newArr });
      const response = await axios.put(`${API_URL}${editModal.reportId}`, { work_permit_issuance: newArr }, {
        headers: user?.token ? { Authorization: `Bearer ${user.token}` } : {},
      });
...
export default CWorkPermitIssuancePage;
