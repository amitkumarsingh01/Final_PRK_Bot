  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editModal.isNew) {
        ...
        newArr = [...report.interior_work_tracking, newEntry];
      } else {
      ...
      console.log('Sending data:', { interior_work_tracking: newArr });
      const response = await axios.put(`${API_URL}${editModal.reportId}`, { interior_work_tracking: newArr }, {
        headers: user?.token ? { Authorization: `Bearer ${user.token}` } : {},
      });
