...
const MaintenanceSchedulePageP: React.FC = () => {
  ...
  const handleAdd = async () => {
    ...
    if (!reportId) return;
    setEditModal({ open: true, isNew: true, item: { ...emptyMaintenanceSchedule }, reportId });
  };
  const handleDelete = async (itemId: string, reportId: string) => {
    ...
    try {
      ...
      await axios.put(`${API_URL}${reportId}`, { maintenance_schedules: newArr }, { headers: { Authorization: `Bearer ${user?.token}` } });
      fetchData();
    } catch (e) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await axios.put(`${API_URL}${editModal.reportId}`, { maintenance_schedules: newArr }, { headers: { Authorization: `Bearer ${user?.token}` } });
...
export default MaintenanceSchedulePageP; 
