...
const emptyQualityPlan: QualityPlan = {
  plan_id: '',
...
const QualityPlanningPage: React.FC = () => {
  ...
  const [viewModal, setViewModal] = useState<{ open: boolean; item: QualityPlan | null }>({ open: false, item: null });
  const [editModal, setEditModal] = useState<{ open: boolean; item: QualityPlan | null; isNew: boolean; reportId: string | null }>({ open: false, item: null, isNew: false, reportId: null });
  ...
  const handleEdit = (item: QualityPlan, reportId: string) => {
    setEditModal({ open: true, item: { ...item }, isNew: false, reportId });
  };
  const handleAdd = async (reportId?: string) => {
    const id = reportId || (await ensureReportForProperty());
    if (!id) return;
    setEditModal({ open: true, isNew: true, item: { ...emptyQualityPlan }, reportId: id });
  };
  const handleDelete = async (itemId: string, reportId: string) => {
    if (!window.confirm('Delete this quality plan?')) return;
    try {
      const report = data.find(r => r.id === reportId);
      if (!report) return;
      const newArr = report.quality_plans.filter(i => i.id !== itemId);
      await axios.put(`${API_URL}${reportId}`, { quality_plans: newArr });
      fetchData();
    } catch (e) {
      setError('Failed to delete');
    }
  };
  const handleView = (item: QualityPlan) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!report) return;
      let newArr: QualityPlan[];
      if (editModal.isNew) {
        newArr = [...report.quality_plans, editModal.item];
      } else {
        newArr = report.quality_plans.map(i =>
          i.id === editModal.item!.id ? editModal.item! : i
        );
      }
      await axios.put(`${API_URL}${editModal.reportId}`, { quality_plans: newArr });
  ...
  return (
    <div className="p-6" style={{ background: '#fff' }}>
      <h2 className="text-2xl font-bold mb-4" style={{ color: orangeDark }}>Quality Planning</h2>
      {/* Property Display */}
        ...
        <table className="min-w-full text-sm">
          <thead>
            <tr style={{ background: orange, color: '#fff' }}>
              ...
              <th className="px-3 py-2 border">Project/Process Name</th>
              <th className="px-3 py-2 border">Objective</th>
              <th className="px-3 py-2 border">Standards</th>
            ...
            {loading ? (
              ...
              <tr>
                <td colSpan={11} className="text-center py-6">
                  <div className="flex items-center justify-center gap-3">
                    ...
                    {isAdmin && (
                      <button onClick={() => handleAdd()} className="ml-2 px-3 py-1 rounded bg-gradient-to-r from-[#E06002] to-[#FB7E03] text-white font-semibold shadow">Add Quality Plan</button>
                    )}
                  </div>
                </td>
              </tr>
              ...
              <>
                {data.flatMap((report, rIdx) =>
                  report.quality_plans.map((item, idx) => (
                    <tr key={item.id || idx} style={{ background: idx % 2 === 0 ? '#fff' : '#FFF7ED' }}>
                      <td className="border px-2 py-1">{idx + 1}</td>
                      <td className="border px-2 py-1">{item.plan_id}</td>
                      <td className="border px-2 py-1">{item.project_process_name}</td>
                      <td className="border px-2 py-1">{item.objective}</td>
                      <td className="border px-2 py-1">{item.standards}</td>
      ...
      {isAdmin && data.length > 0 && (
        ...
        >
          <Plus size={18} className="mr-2" /> Add Quality Plan
        </button>
      )}
          ...
          <div className="bg-white rounded-lg p-6 max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold text-gray-900">
                {editModal.isNew ? 'Add' : 'Edit'} Quality Plan
              </h3>
              <button
            ...
            <form onSubmit={e => { e.preventDefault(); handleSave(); }}>
              <div className="grid grid-cols-2 gap-3">
                ...
                <input className="border rounded px-3 py-2" placeholder="Project/Process Name" value={editModal.item.project_process_name} onChange={e => setEditModal(m => m && { ...m, item: { ...m.item!, project_process_name: e.target.value } })} required />
                <textarea className="border rounded px-3 py-2" placeholder="Objective" value={editModal.item.objective} onChange={e => setEditModal(m => m && { ...m, item: { ...m.item!, objective: e.target.value } })} required rows={3} />
                <textarea className="border rounded px-3 py-2" placeholder="Standards" value={editModal.item.standards} onChange={e => setEditModal(m => m && { ...m, item: { ...m.item!, standards: e.target.value } })} required rows={3} />
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto">
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              ...
              <div><b>Project/Process Name:</b> {viewModal.item.project_process_name}</div>
              <div className="col-span-2"><b>Objective:</b> {viewModal.item.objective}</div>
              <div className="col-span-2"><b>Standards:</b> {viewModal.item.standards}</div>
