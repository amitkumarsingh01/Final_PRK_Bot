...
const SlaMonitoringPage: React.FC = () => {
  console.log('🚀 SlaMonitoring: Component initialized');
  const { user } = useAuth();
  console.log('👤 SlaMonitoring: User loaded', { userId: user?.userId });
  ...
  const handleEdit = (item: SlaMonitoring, reportId: string) => {
  ...
  const handleView = (item: SlaMonitoring) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editModal.isNew) {
        newArr = [...(report.sla_monitoring || []), editModal.item];
      } else {
        newArr = (report.sla_monitoring || []).map((i: SlaMonitoring) =>
            ...
            {loading ? (
              ...
              <>
                {data.flatMap((report, rIdx) =>
                  (report.sla_monitoring || []).map((item, idx) => (
          ...
          <div className="bg-white rounded-lg p-6 max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold text-gray-900">
                {editModal.isNew ? 'Add' : 'Edit'} SLA Monitoring Entry
              </h3>
              <button
