...
const AMCWarrantyTrackerPage: React.FC = () => {
  ...
  const handleAdd = async () => {
    ...
    if (!reportId) return;
    setEditModal({ open: true, isNew: true, item: { ...emptyAmcWarranty }, reportId });
  };
  const handleDelete = async (itemId: string, reportId: string) => {
    ...
    try {
      ...
      await axios.put(`${API_URL}${reportId}`, { amc_warranties: newArr }, { headers: { Authorization: `Bearer ${user?.token}` } });
      fetchData();
    } catch (e) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await axios.put(`${API_URL}${editModal.reportId}`, { amc_warranties: newArr }, { headers: { Authorization: `Bearer ${user?.token}` } });
