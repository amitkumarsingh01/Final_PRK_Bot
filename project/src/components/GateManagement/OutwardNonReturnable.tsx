...
interface OutwardNonReturnable {
  id?: string;
  report_id?: string;
  item_id: string;
...
const emptyOutward: OutwardNonReturnable = {
  item_id: '',
  item_description: '',
  quantity: 1,
  recipient_name: '',
...
const OutwardNonReturnablePage: React.FC = () => {
  const { user } = useAuth();
  ...
  const [viewModal, setViewModal] = useState<{ open: boolean; item: OutwardNonReturnable | null }>({ open: false, item: null });
  const [editModal, setEditModal] = useState<{ open: boolean; item: OutwardNonReturnable | null; isNew: boolean; reportId: string | null }>({ open: false, item: null, isNew: false, reportId: null });

  // Fetch properties and user's default property_id
  useEffect(() => {
    // No dropdown; we don't need to fetch all properties for selection
    // Keep this in case some views rely on the list (but UI won't show it)
  ...
  // CRUD handlers
  const handleEdit = (item: OutwardNonReturnable, reportId: string) => {
  ...
  const handleDelete = async (itemId: string, reportId: string) => {
    ...
    try {
      ...
      if (!report) return;
      // Remove the entry
      const newArr = report.outward_non_returnable.filter(i => i.id !== itemId);
      // Update the report
      await axios.put(`${API_URL}${reportId}`, { outward_non_returnable: newArr });
      fetchData(selectedPropertyId);
    } catch (e) {
      setError('Failed to delete');
    }
  };
  const handleView = (item: OutwardNonReturnable) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!report) return;
      let newArr: OutwardNonReturnable[];
      if (editModal.isNew) {
        newArr = [...report.outward_non_returnable, editModal.item];
      } else {
        newArr = report.outward_non_returnable.map(i =>
          i.id === editModal.item!.id ? editModal.item! : i
        );
      }
      await axios.put(`${API_URL}${editModal.reportId}`, { outward_non_returnable: newArr });
  ...
  // Main render
    ...
    <div className="p-6" style={{ background: '#fff' }}>
      <h2 className="text-2xl font-bold mb-4" style={{ color: orangeDark }}>Outward Non-Returnable Management</h2>
            ...
            {loading ? (
              ...
              <>
                {data.flatMap((report, rIdx) =>
                  report.outward_non_returnable.map((item, idx) => (
      ...
      {isCadmin && data.length > 0 && (
        ...
        >
          <Plus size={18} className="mr-2" /> Add Outward Non-Returnable Entry
        </button>
      )}
          ...
          <div className="bg-white rounded-lg p-6 max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold text-gray-900">
                {editModal.isNew ? 'Add' : 'Edit'} Outward Non-Returnable Entry
              </h3>
              <button
...
export default OutwardNonReturnablePage;
