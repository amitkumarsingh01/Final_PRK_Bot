...
const emptyInward: InwardReturnable = {
  item_id: '',
  item_description: '',
  quantity: 1,
  supplier_name: '',
...
const InwardReturnablePage: React.FC = () => {
  const { user } = useAuth();
  ...
  const [viewModal, setViewModal] = useState<{ open: boolean; item: InwardReturnable | null }>({ open: false, item: null });
  const [editModal, setEditModal] = useState<{ open: boolean; item: InwardReturnable | null; isNew: boolean; reportId: string | null }>({ open: false, item: null, isNew: false, reportId: null });
  ...
  // CRUD handlers
  const handleEdit = (item: InwardReturnable, reportId: string) => {
  ...
  const handleView = (item: InwardReturnable) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!report) return;
      let newArr: InwardReturnable[];
      if (editModal.isNew) {
        newArr = [...report.inward_returnable, editModal.item];
      } else {
        newArr = report.inward_returnable.map(i =>
  ...
  // Main render
    ...
    <div className="p-6" style={{ background: '#fff' }}>
            ...
            {loading ? (
              ...
              <>
                {data.flatMap((report, rIdx) =>
                  report.inward_returnable.map((item, idx) => (
      ...
      {isCadmin && data.length > 0 && (
        ...
        >
          <Plus size={18} className="mr-2" /> Add Inward Returnable Entry
        </button>
      )}
          ...
          <div className="bg-white rounded-lg p-6 max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold text-gray-900">
                {editModal.isNew ? 'Add' : 'Edit'} Inward Returnable Entry
              </h3>
              <button
...
export default InwardReturnablePage;
