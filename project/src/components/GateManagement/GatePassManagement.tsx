...
const emptyGatePass: GatePassManagement = {
  pass_id: '',
  name: '',
  contact_number: '',
  purpose: '',
...
const GatePassManagementPage: React.FC = () => {
  const { user } = useAuth();
  ...
  const handleView = (item: GatePassManagement) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!report) return;
      let newArr: GatePassManagement[];
      if (editModal.isNew) {
        newArr = [...report.gate_pass_management, editModal.item];
      } else {
        newArr = report.gate_pass_management.map(i =>
    ...
    <div className="p-6" style={{ background: '#fff' }}>
        ...
        <table className="min-w-full text-sm">
          <thead>
            <tr style={{ background: orange, color: '#fff' }}>
              ...
              <th className="px-3 py-2 border">Pass ID</th>
              ...
              <th className="px-3 py-2 border">Exit Date</th>
            ...
            {loading ? (
              ...
              <>
                {data.flatMap((report, rIdx) =>
                  report.gate_pass_management.map((item, idx) => (
                    <tr key={item.id || idx} style={{ background: idx % 2 === 0 ? '#fff' : '#FFF7ED' }}>
                      <td className="border px-2 py-1">{idx + 1}</td>
                      <td className="border px-2 py-1">{item.pass_id}</td>
                      ...
                      <td className="border px-2 py-1">{item.exit_date || '-'}</td>
      ...
      {isCadmin && data.length > 0 && (
        ...
        >
          <Plus size={18} className="mr-2" /> Add Gate Pass Entry
        </button>
      )}
          ...
          <div className="bg-white rounded-lg p-6 max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold text-gray-900">
                {editModal.isNew ? 'Add' : 'Edit'} Gate Pass Entry
              </h3>
              <button
            ...
            <form onSubmit={e => { e.preventDefault(); handleSave(); }}>
              <div className="grid grid-cols-2 gap-3">
                <input className="border rounded px-3 py-2" placeholder="Pass ID" value={editModal.item.pass_id} onChange={e => setEditModal(m => m && { ...m, item: { ...m.item!, pass_id: e.target.value } })} required />
                ...
                <input className="border rounded px-3 py-2" placeholder="Exit Date" type="date" value={editModal.item.exit_date || ''} onChange={e => setEditModal(m => m && { ...m, item: { ...m.item!, exit_date: e.target.value } })} />
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              <div><b>Pass ID:</b> {viewModal.item.pass_id}</div>
              ...
              <div><b>Exit Date:</b> {viewModal.item.exit_date || '-'}</div>
...
export default GatePassManagementPage;
