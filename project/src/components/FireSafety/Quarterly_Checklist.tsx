...
interface FireSafetyReport {
  id: string;
  property_id: string;
  quarterly_checklists: QuarterlyChecklist[];
}
...
const emptyQuarterlyChecklist: QuarterlyChecklist = {
  Checklist_ID: '',
  Site_Name: '',
  Quarter: '',
  Year: '',
...
const QuarterlyChecklistPage: React.FC = () => {
  console.log('🚀 QuarterlyChecklist: Component initialized');
  const { user } = useAuth();
  console.log('👤 QuarterlyChecklist: User loaded', { userId: user?.userId });
  ...
  const handleDelete = async (itemId: string, reportId: string) => {
    if (!window.confirm('Delete this quarterly checklist record?')) return;
    try {
      const report = data.find(r => r.id === reportId);
      if (!report) return;
      const newArr = report.quarterly_checklists.filter(i => i.id !== itemId);
      await axios.put(`${API_URL}${reportId}`, { 
        Fire_Safety_Management: { Quarterly_Checklist: newArr }
      });
      fetchData();
    } catch (e) {
  ...
  const handleView = (item: QuarterlyChecklist) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!report) return;
      let newArr: QuarterlyChecklist[];
      if (editModal.isNew) {
        newArr = [...report.quarterly_checklists, editModal.item];
      } else {
        newArr = report.quarterly_checklists.map(i =>
  ...
  return (
    <div className="p-6" style={{ background: '#fff' }}>
      <h2 className="text-2xl font-bold mb-4" style={{ color: orangeDark }}>Quarterly Checklist</h2>
        ...
        <table className="min-w-full text-sm">
          <thead>
            <tr style={{ background: orange, color: '#fff' }}>
              ...
              <th className="px-3 py-2 border">Quarter</th>
              ...
              const rows = data.flatMap((report, rIdx) =>
                report.quarterly_checklists.map((item, idx) => (
                  <tr key={item.id || idx} style={{ background: idx % 2 === 0 ? '#fff' : '#FFF7ED' }}>
                    ...
                    <td className="border px-2 py-1">{item.Quarter}</td>
          ...
          <div className="bg-white rounded-lg p-6 max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold text-gray-900">
                {editModal.isNew ? 'Add' : 'Edit'} Quarterly Checklist
              </h3>
              <button
            ...
            <form onSubmit={e => { e.preventDefault(); handleSave(); }}>
              <div className="grid grid-cols-2 gap-3">
                ...
                <input className="border rounded px-3 py-2" placeholder="Quarter" value={editModal.item.Quarter} onChange={e => setEditModal(m => m && { ...m, item: { ...m.item!, Quarter: e.target.value } })} required />
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto">
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              ...
              <div><b>Quarter:</b> {viewModal.item.Quarter}</div>
...
export default QuarterlyChecklistPage;