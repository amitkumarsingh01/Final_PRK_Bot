...
const emptyWeeklyChecklist: WeeklyChecklist = {
  Checklist_ID: '',
  Site_Name: '',
  Week: '',
  Year: '',
...
const WeeklyChecklistPage: React.FC = () => {
  console.log('🚀 WeeklyChecklist: Component initialized');
  const { user } = useAuth();
  console.log('👤 WeeklyChecklist: User loaded', { userId: user?.userId });
  const [data, setData] = useState<FireSafetyReport[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [isAdmin, setIsAdmin] = useState(false);
  const [viewModal, setViewModal] = useState<{ open: boolean; item: WeeklyChecklist | null }>({ open: false, item: null });
  const [editModal, setEditModal] = useState<{ open: boolean; item: WeeklyChecklist | null; isNew: boolean; reportId: string | null }>({ open: false, item: null, isNew: false, reportId: null });
  ...
  const handleEdit = (item: WeeklyChecklist, reportId: string) => {
  ...
  const handleAdd = async (reportId?: string) => {
    ...
    setEditModal({
      open: true,
      isNew: true,
      item: { ...emptyWeeklyChecklist },
      reportId: id,
    });
  };

  const handleDelete = async (itemId: string, reportId: string) => {
    if (!window.confirm('Delete this weekly checklist record?')) return;
    try {
      const report = data.find(r => r.id === reportId);
      if (!report) return;
      const newArr = report.weekly_checklists.filter(i => i.id !== itemId);
      await axios.put(`${API_URL}${reportId}`, { 
        Fire_Safety_Management: { Weekly_Checklist: newArr }
      });
      fetchData();
    } catch (e) {
  ...
  const handleView = (item: WeeklyChecklist) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!report) return;
      let newArr: WeeklyChecklist[];
      if (editModal.isNew) {
        newArr = [...report.weekly_checklists, editModal.item];
      } else {
        newArr = report.weekly_checklists.map(i =>
  ...
  return (
    <div className="p-6" style={{ background: '#fff' }}>
      <h2 className="text-2xl font-bold mb-4" style={{ color: orangeDark }}>Weekly Checklist</h2>
                  ...
                  <tr>
                    <td colSpan={10} className="text-center py-6">
                      <div className="flex items-center justify-center gap-3">
                        <span>No weekly checklist records found</span>
                        {isAdmin && (
                          <button onClick={() => handleAdd()} className="ml-2 px-3 py-1 rounded bg-gradient-to-r from-[#E06002] to-[#FB7E03] text-white font-semibold shadow">Add Weekly Checklist</button>
                        )}
                      </div>
                    </td>
                  </tr>
      ...
      {isAdmin && (
        ...
        >
          <Plus size={18} className="mr-2" /> Add Weekly Checklist
        </button>
      )}
          ...
          <div className="bg-white rounded-lg p-6 max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold text-gray-900">
                {editModal.isNew ? 'Add' : 'Edit'} Weekly Checklist
              </h3>
              <button
            ...
            <form onSubmit={e => { e.preventDefault(); handleSave(); }}>
              <div className="grid grid-cols-2 gap-3">
                ...
                <input className="border rounded px-3 py-2" placeholder="Week" value={editModal.item.Week} onChange={e => setEditModal(m => m && { ...m, item: { ...m.item!, Week: e.target.value } })} required />
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto">
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              ...
              <div><b>Week:</b> {viewModal.item.Week}</div>
