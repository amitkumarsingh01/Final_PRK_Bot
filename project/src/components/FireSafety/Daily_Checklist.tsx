...
const DailyChecklistPage: React.FC = () => {
  console.log('🚀 DailyChecklist: Component initialized');
  const { user } = useAuth();
  console.log('👤 DailyChecklist: User loaded', { userId: user?.userId });
                  ...
                  <tr>
                    <td colSpan={10} className="text-center py-6">
                      <div className="flex items-center justify-center gap-3">
                        <span>No daily checklist records found</span>
                        {isAdmin && (
                          <button onClick={() => handleAdd()} className="ml-2 px-3 py-1 rounded bg-gradient-to-r from-[#E06002] to-[#FB7E03] text-white font-semibold shadow">Add Daily Checklist</button>
                        )}
                      </div>
                    </td>
                  </tr>
