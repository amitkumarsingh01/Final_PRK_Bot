...
const emptyCategory: ProcurementCategory = {
  Category_ID: '',
  Category_Name: '',
  Description: '',
  Budget_Allocation: 0,
...
const ProcurementCategoriesPage: React.FC = () => {
  ...
  const handleDelete = async (categoryId: string, reportId: string) => {
    ...
    try {
      ...
      if (report) {
        const updated = report.procurement_categories.filter(c => c.id !== categoryId);
        await axios.put(`${API_URL}${reportId}`, {
          property_id: user?.propertyId,
          Procurement_Management: {
            Procurement_Categories: updated,
          },
  ...
  const handleSave = async () => {
    if (!editModal.category || !editModal.reportId) return;
    try {
      const report = data.find(r => r.id === editModal.reportId);
      if (report) {
        let updated: ProcurementCategory[];
        if (editModal.isNew) {
          const newCategory = { ...editModal.category, id: `temp_${Date.now()}` };
          updated = [...report.procurement_categories, newCategory];
        } else {
          updated = report.procurement_categories.map(c => (c.id === editModal.category!.id ? editModal.category! : c));
        }
  ...
  const handleAdd = async (reportId?: string) => {
    ...
    if (!id) return;
    setEditModal({ open: true, category: { ...emptyCategory }, isNew: true, reportId: id });
  };
        ...
        <div className="bg-white rounded-lg shadow-md p-6 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
            ...
            <div className="bg-gradient-to-r from-yellow-400 to-yellow-600 text-white p-4 rounded-lg">
              <div className="text-2xl font-bold">{categories.filter(c => c.Status === 'Pending').length}</div>
              <div className="text-sm">Pending</div>
            </div>
            <div className="bg-gradient-to-r from-blue-400 to-blue-600 text-white p-4 rounded-lg">
              <div className="text-2xl font-bold">₹{categories.reduce((sum, c) => sum + c.Budget_Allocation, 0).toLocaleString()}</div>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {categories.map((c) => (
                    <tr key={c.id} className="hover:bg-gray-50">
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{c.Category_ID}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{c.Category_Name}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">₹{c.Budget_Allocation.toLocaleString()}</td>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <div className="flex items-center space-x-2">
                          <button onClick={() => handleView(c)} className="text-blue-600 hover:text-blue-900">
                            <Eye size={16} />
                          </button>
                          {isAdmin && (
                            <>
                              <button onClick={() => handleEdit(c, c.report_id!)} className="text-orange-600 hover:text-orange-900">
                                <Pencil size={16} />
                              </button>
                              <button onClick={() => handleDelete(c.id!, c.report_id!)} className="text-red-600 hover:text-red-900">
                                <Trash2 size={16} />
                              </button>
                            </>
                          )}
      ...
      {viewModal.open && viewModal.category && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-semibold text-gray-900">View Category</h3>
                <button onClick={() => setViewModal({ open: false, category: null })} className="text-gray-400 hover:text-gray-600">
                  <X size={24} />
                </button>
              </div>
      ...
      {editModal.open && editModal.category && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-semibold text-gray-900">{editModal.isNew ? 'Add Category' : 'Edit Category'}</h3>
                <button onClick={() => setEditModal({ open: false, category: null, isNew: false, reportId: null })} className="text-gray-400 hover:text-gray-600">
                  <X size={24} />
                </button>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700">Category ID</label>
                  <input type="text" value={editModal.category.Category_ID} onChange={(e) => setEditModal({ ...editModal, category: { ...editModal.category!, Category_ID: e.target.value } })} className="mt-1 block w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-orange-500" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700">Name</label>
                  <input type="text" value={editModal.category.Category_Name} onChange={(e) => setEditModal({ ...editModal, category: { ...editModal.category!, Category_Name: e.target.value } })} className="mt-1 block w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-orange-500" />
                </div>
                <div className="col-span-2">
                  <label className="block text-sm font-medium text-gray-700">Description</label>
                  <textarea value={editModal.category.Description} onChange={(e) => setEditModal({ ...editModal, category: { ...editModal.category!, Description: e.target.value } })} rows={2} className="mt-1 block w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-orange-500" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700">Budget Allocation</label>
                  <input type="number" step="0.01" value={editModal.category.Budget_Allocation} onChange={(e) => setEditModal({ ...editModal, category: { ...editModal.category!, Budget_Allocation: parseFloat(e.target.value) || 0 } })} className="mt-1 block w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-orange-500" />
                </div>
                <div className="col-span-2">
                  <label className="block text-sm font-medium text-gray-700">Items/Services</label>
                  <textarea value={editModal.category.Items_Services} onChange={(e) => setEditModal({ ...editModal, category: { ...editModal.category!, Items_Services: e.target.value } })} rows={2} className="mt-1 block w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-orange-500" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700">Responsible Person</label>
                  <input type="text" value={editModal.category.Responsible_Person} onChange={(e) => setEditModal({ ...editModal, category: { ...editModal.category!, Responsible_Person: e.target.value } })} className="mt-1 block w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-orange-500" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700">Status</label>
                  <select value={editModal.category.Status} onChange={(e) => setEditModal({ ...editModal, category: { ...editModal.category!, Status: e.target.value } })} className="mt-1 block w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-orange-500">
                ...
                <div className="col-span-2">
                  <label className="block text-sm font-medium text-gray-700">Remarks</label>
                  <textarea value={editModal.category.Remarks} onChange={(e) => setEditModal({ ...editModal, category: { ...editModal.category!, Remarks: e.target.value } })} rows={2} className="mt-1 block w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-orange-500" />
                </div>
              </div>
              <div className="flex justify-end space-x-3 mt-6">
                <button onClick={() => setEditModal({ open: false, category: null, isNew: false, reportId: null })} className="px-4 py-2 border border-gray-300 rounded-md text-gray-700 hover:bg-gray-50">Cancel</button>
