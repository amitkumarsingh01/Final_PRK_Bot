...
const AmcCompliancePage: React.FC = () => {
  ...
  const [editModal, setEditModal] = useState<{ open: boolean; item: AmcComplianceFormat | null; isNew: boolean; reportId: string | null; index?: number }>({ open: false, item: null, isNew: false, reportId: null });
  ...
  const handleEdit = (item: AmcComplianceFormat, reportId: string, index?: number) => {
  ...
  const handleDelete = async (itemId: string | undefined, reportId: string, index?: number) => {
    ...
    try {
      ...
      if (!report) return;
      const current = (report.cctv_audit_data?.AMC_Compliance_Format || []);
      const newArr = itemId ? current.filter((i: AmcComplianceFormat) => i.id !== itemId) : current.filter((_, idx: number) => idx !== (index ?? -1));
      await axios.put(`${API_URL}${reportId}`, {
        CCTV_Audit: { ...(report.cctv_audit_data || {}), AMC_Compliance_Format: newArr }
    ...
    try {
      ...
      if (editModal.isNew) {
        ...
        if (hasId) {
          ...
          newArr = [...(report.cctv_audit_data?.AMC_Compliance_Format || [])];
