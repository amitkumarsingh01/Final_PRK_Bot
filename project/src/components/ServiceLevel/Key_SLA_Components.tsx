...
interface SlaReport {
  ...
  key_components?: KeySlaComponent[];
}
...
const KeySlaComponentsPage: React.FC = () => {
  ...
  const normalizeReports = (arr: any[]): SlaReport[] => {
  ...
  const handleAdd = async (reportId?: string) => {
    ...
    if (!id) return;
    setEditModal({ open: true, isNew: true, item: { ...emptyKeySlaComponent }, reportId: id });
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editModal.isNew) {
        ...
        newArr = (report.key_sla_components || []).map((i: KeySlaComponent) => (i.id === editModal.item!.id ? editModal.item! : i));
      }
      await axios.put(
  ...
  const items = data.flatMap(r => (r.key_sla_components || []).map(i => ({ ...i, report_id: r.id })));

  return (
    <div className="p-6" style={{ background: '#fff' }}>
      <h2 className="text-2xl font-bold mb-4" style={{ color: orangeDark }}>Key SLA Components</h2>

      {/* Property Display (no dropdown) */}
            ...
            {loading ? (
              ...
              <tr>
                <td colSpan={11} className="text-center py-8">
                  <div className="text-gray-500 mb-3">No components found</div>
      ...
      {isAdmin && items.length > 0 && (
        <button
          onClick={() => handleAdd(items[0].report_id)}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              ...
              <button onClick={() => setEditModal({ open: false, item: null, isNew: false, reportId: null })} className="text-gray-400 hover:text-gray-600">
                <X size={24} />
              </button>
            </div>
      ...
      {viewModal.open && viewModal.item && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold text-gray-900">Key SLA Component Details</h3>
              <button onClick={() => setViewModal({ open: false, item: null })} className="text-gray-400 hover:text-gray-600">
                <X size={24} />
              </button>
            </div>
