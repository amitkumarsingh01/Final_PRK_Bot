...
const SlaPlanningAndDefinitionPage: React.FC = () => {
  ...
  const handleAdd = async (reportId?: string) => {
    ...
    if (!id) return;
    setEditModal({ open: true, item: { ...emptySlaPlanningAndDefinition }, isNew: true, reportId: id });
  };
  ...
  const handleDelete = async (itemId: string, reportId: string) => {
    ...
    try {
      ...
      await axios.put(`${API_URL}${reportId}`, { sla_planning_and_definition: newArr }, user?.token ? { headers: { Authorization: `Bearer ${user.token}` } } : undefined);
      fetchData();
    } catch (e) {
  ...
  const handleView = (item: SlaPlanningAndDefinition) => setViewModal({ open: true, item });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editModal.isNew) newArr = [...(report.sla_planning_and_definition || []), editModal.item];
      else newArr = (report.sla_planning_and_definition || []).map((i: SlaPlanningAndDefinition) => (i.id === editModal.item!.id ? editModal.item! : i));
      await axios.put(`${API_URL}${editModal.reportId}`, { sla_planning_and_definition: newArr }, user?.token ? { headers: { Authorization: `Bearer ${user.token}` } } : undefined);
  ...
  const items = data.flatMap(r => (r.sla_planning_and_definition || []).map(i => ({ ...i, report_id: r.id })));
    ...
    <div className="p-6" style={{ background: '#fff' }}>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold text-gray-900">{editModal.isNew ? 'Add' : 'Edit'} SLA Planning Entry</h3>
      ...
      {viewModal.open && viewModal.item && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold text-gray-900">SLA Planning Details</h3>
